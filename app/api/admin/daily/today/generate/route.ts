...
import { createTodayDailyChallengeIfMissing } from "@/lib/daily-challenge";
...
export async function POST() {
  ...
  try {
    const result = await createTodayDailyChallengeIfMissing();
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Failed to generate today's daily challenge" },
      { status: 500 }
    );
  }
}
